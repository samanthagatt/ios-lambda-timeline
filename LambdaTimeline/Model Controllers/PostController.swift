...
class PostController {
    ...
    func createPost(with title: String, coordinate: CLLocationCoordinate2D? = nil, ofType mediaType: MediaType, mediaData: Data, ratio: CGFloat? = nil, completion: @escaping (Bool) -> Void = { _ in }) {
        ...
        store(mediaData: mediaData, mediaType: mediaType) { (mediaURL) in
            ...
            let mediaPost = Post(title: title, coordinate: coordinate, mediaType: mediaType, mediaURL: mediaURL, ratio: ratio, author: author)
            
            self.postsRef.childByAutoId().setValue(mediaPost.dictionaryRepresentation) { (error, ref) in
                if let error = error {
    ...
    func addComment(text: String? = nil, audioURL: URL? = nil, to post: inout Post) {
        ...
        if let audioURLComment = audioURL {
           comment = Comment(audioURL: audioURLComment, author: author)
        } else {
